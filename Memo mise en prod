# DRAFT — Fiche mémo · Mise en prod Collegium Magistrorum

## 0) Préparer le repo (à faire une fois)
...
## 2) Déployer sur le serveur (`/opt/collegium-bot`)
    ./deploy.sh latest
    docker compose logs -f

---

## 3) Mettre à jour (nouveau code)
...
## 7) Entretien (à utiliser avec précaution)
    docker ps
    docker images
    docker system df
...
## 9) Ordre ultra-court à mémoriser
1. Coder → push  
2. CI build/push → image dans GHCR  
3. Serveur : `./deploy.sh <tag>` → `logs -f`  
4. Vérifier sur Discord (slash-commands)  
5. Versionner : `git tag vX.Y.Z` → rollback facile

---

## 10) Astuces copy/paste (si ton éditeur crée des liens)
